...
for k in range(1,5):
    ...
    LAMval = [0,0.01,0.03,0.05]#,0.0003,0.0005,0.0007]#,0.05,0.075,0.1,0.25,0.5,0.75,1,1.5]

    BetaVal = [0,0.03,0.05,0.075]
    ...
    for lam in LAMval:
        for beta in BetaVal:
            ...
            ruta = './restoration/P242/denoised_P242_b'
            fname = ruta + str(beta) + '_L'+ str(Lambda) + nameimg 
            out_img_avg_pil.save(fname)

            #Save best image 
            imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
            fname = './restoration/P242/BestPnsr_P242_b' +  str(beta) +'_L'+ str(Lambda) + nameimg 
