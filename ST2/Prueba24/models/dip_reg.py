...
class EuclideanDiracColor(nn.Module):
	...
	def forward(self, input):	
		...
		norm = 0.00001*torch.ones((self.height,self.width)).type(self.dtype)
		
		norm = norm + torch.square(red_x + green_y)	+ torch.square(green_x-red_y)
		norm = norm	+ torch.square(0.5*red_xx)	+ torch.square(0.5*red_yy)
		norm = norm	+ torch.square(0.5*green_xx) + torch.square(0.5*green_yy) 

		norm = norm + torch.square(blue_x + red_y) + torch.square(red_x - blue_y) 	
		norm = norm	+ torch.square(0.5*blue_xx) + torch.square(0.5*blue_yy)
		norm = norm	+ torch.square(0.5*red_xx)+ torch.square(0.5*red_yy) 
		
		norm = norm	+ torch.square(green_x + blue_y) + torch.square(blue_x - green_y)  	
		norm = norm	+ torch.square(0.5*green_xx) + torch.square(0.5*green_yy)
		norm = norm	+ torch.square(0.5*blue_xx) + torch.square(0.5*blue_yy)
...
class EuclideanDiracColor2(nn.Module):
	...
	def forward(self, input):	
		...
		norm1 = 0.00001*torch.ones((self.height,self.width)).type(self.dtype) 

		sumaGrad = norm_grad_red_squared + norm_grad_green_squared  + norm_grad_blue_squared 
		suma = sumaGrad + (torch.mul(red_x,green_y)-torch.mul(red_y,green_x)) + (torch.mul(blue_x,red_y)-torch.mul(blue_y,red_x)) + (torch.mul(green_x,blue_y)-torch.mul(green_y,blue_x))
		
		norm2 = norm1	+ torch.maximum(torch.zeros((self.height,self.width)).type(self.dtype),suma)
...
class EuclideanDiracColor3(nn.Module):
	def __init__(self,input_depth, dtype, pad, height, width, upsample_mode, n_channels=3, act_fun='LeakyReLU', skip_n33d=128, skip_n33u=128, skip_n11=4, num_scales=5, downsample_mode='stride'):
		super(EuclideanDiracColor3,self).__init__()
		self.net = skip(input_depth, n_channels, num_channels_down = [skip_n33d]*num_scales if isinstance(skip_n33d, int) else skip_n33d,
                                            num_channels_up =   [skip_n33u]*num_scales if isinstance(skip_n33u, int) else skip_n33u,
                                            num_channels_skip = [skip_n11]*num_scales if isinstance(skip_n11, int) else skip_n11, 
                                            upsample_mode=upsample_mode, downsample_mode=downsample_mode,
	...
	def forward(self, input):	
		...
		suma = norm_grad_red_squared + norm_grad_green_squared  + norm_grad_blue_squared 
		suma2 = (torch.mul(red_x,green_y)-torch.mul(red_y,green_x)) + (torch.mul(blue_x,red_y)-torch.mul(blue_y,red_x)) + (torch.mul(green_x,blue_y)-torch.mul(green_y,blue_x))
		
		norm = torch.div(norm, torch.maximum(torch.ones((self.height,self.width)).type(self.dtype),suma//suma2))
		norm = torch.sqrt(norm)
		#norm = torch.sqrt(0.00001*torch.ones((self.height,self.width)).type(self.dtype) + torch.maximum(torch.zeros((self.height,self.width)).type(self.dtype),norm_grad_red_squared + norm_grad_green_squared + norm_grad_blue_squared //
		#	+ (torch.mul(red_x,green_y)-torch.mul(red_y,green_x)) + (torch.mul(blue_x,red_y)-torch.mul(blue_y,red_x)) + (torch.mul(green_x,blue_y)-torch.mul(green_y,blue_x)))) 
...
class EuclideanDiracColor4(nn.Module):
	def __init__(self,input_depth, dtype, pad, height, width, upsample_mode, n_channels=3, act_fun='LeakyReLU', skip_n33d=128, skip_n33u=128, skip_n11=4, num_scales=5, downsample_mode='stride'):
		super(EuclideanDiracColor4,self).__init__()
		self.net = skip(input_depth, n_channels, num_channels_down = [skip_n33d]*num_scales if isinstance(skip_n33d, int) else skip_n33d,
                                            num_channels_up =   [skip_n33u]*num_scales if isinstance(skip_n33u, int) else skip_n33u,
                                            num_channels_skip = [skip_n11]*num_scales if isinstance(skip_n11, int) else skip_n11, 
                                            upsample_mode=upsample_mode, downsample_mode=downsample_mode,
	...
	def forward(self, input):	
		...
		norm2 = norm1	+ torch.maximum(0.00001*torch.ones((self.height,self.width)).type(self.dtype) ,suma)
		norm = torch.sqrt(norm2)
		#norm = torch.sqrt(0.00001*torch.ones((self.height,self.width)).type(self.dtype) + torch.maximum(torch.zeros((self.height,self.width)).type(self.dtype),norm_grad_red_squared + norm_grad_green_squared + norm_grad_blue_squared //
		#	+ (torch.mul(red_x,green_y)-torch.mul(red_y,green_x)) + (torch.mul(blue_x,red_y)-torch.mul(blue_y,red_x)) + (torch.mul(green_x,blue_y)-torch.mul(green_y,blue_x)))) 
...
class EuclideanDiracColor5(nn.Module):
	def __init__(self,input_depth, dtype, pad, height, width, upsample_mode, n_channels=3, act_fun='LeakyReLU', skip_n33d=128, skip_n33u=128, skip_n11=4, num_scales=5, downsample_mode='stride'):
		super(EuclideanDiracColor5,self).__init__()
		self.net = skip(input_depth, n_channels, num_channels_down = [skip_n33d]*num_scales if isinstance(skip_n33d, int) else skip_n33d,
                                            num_channels_up =   [skip_n33u]*num_scales if isinstance(skip_n33u, int) else skip_n33u,
                                            num_channels_skip = [skip_n11]*num_scales if isinstance(skip_n11, int) else skip_n11, 
                                            upsample_mode=upsample_mode, downsample_mode=downsample_mode,
	...
	def forward(self, input):	
		...
		norm = torch.sqrt(0.00001*torch.ones((self.height,self.width)).type(self.dtype) + torch.maximum(torch.zeros((self.height,self.width)).type(self.dtype),norm_grad_red_squared + norm_grad_green_squared + norm_grad_blue_squared + (torch.mul(red_x,green_y)-torch.mul(red_y,green_x)) + (torch.mul(blue_x,red_y)-torch.mul(blue_y,red_x)) + (torch.mul(green_x,blue_y)-torch.mul(green_y,blue_x)))) 

		norm = torch.unsqueeze(norm,dim=0)

		return output,norm

class EuclideanDiracColor6(nn.Module):
	def __init__(self,input_depth, dtype, pad, height, width, upsample_mode, n_channels=3, act_fun='LeakyReLU', skip_n33d=128, skip_n33u=128, skip_n11=4, num_scales=5, downsample_mode='stride'):
		super(EuclideanDiracColor6,self).__init__()
		self.net = skip(input_depth, n_channels, num_channels_down = [skip_n33d]*num_scales if isinstance(skip_n33d, int) else skip_n33d,
                                            num_channels_up =   [skip_n33u]*num_scales if isinstance(skip_n33u, int) else skip_n33u,
                                            num_channels_skip = [skip_n11]*num_scales if isinstance(skip_n11, int) else skip_n11, 
                                            upsample_mode=upsample_mode, downsample_mode=downsample_mode,
	...
	def forward(self, input):	
		...
		norm = torch.div(norm, torch.maximum(torch.ones((self.height,self.width)).type(self.dtype),torch.div(suma, suma2, rounding_mode='floor')),rounding_mode='floor')
		norm = torch.sqrt(norm)
		#norm = torch.sqrt(0.00001*torch.ones((self.height,self.width)).type(self.dtype) + torch.maximum(torch.zeros((self.height,self.width)).type(self.dtype),norm_grad_red_squared + norm_grad_green_squared + norm_grad_blue_squared //
		#	+ (torch.mul(red_x,green_y)-torch.mul(red_y,green_x)) + (torch.mul(blue_x,red_y)-torch.mul(blue_y,red_x)) + (torch.mul(green_x,blue_y)-torch.mul(green_y,blue_x)))) 
