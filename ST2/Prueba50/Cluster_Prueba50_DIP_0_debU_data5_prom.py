...
#for k in listaRep:#for k in range(1,25):#25
	...
	fila_prom = np.mean(plotY,axis=0)
	plotY = np.concatenate((plotY,[fila_prom]),axis= 0)
	...
	fname = ruta + 'P'+ Nprueb + denb +'PNSR' + name # + 't'+ str(t) 
	np.savetxt(fname, Y.T, delimiter =",",fmt ='% s')
				
	fila_prom = np.mean(maxPnsr,axis=0)
	maxPnsr = np.concatenate((maxPnsr,[fila_prom]),axis= 0)
	...
	fname = ruta  + 'P'+ Nprueb + denb +'MAXval'  + name #+ 't'+ str(t)
