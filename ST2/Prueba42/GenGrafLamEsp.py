...
lim = 25
LambEsp = 0.03
listaRep = [2,3,4,6,12,13,14,16,18,20,24]
...
freqMX, q = (np.max(counts), np.argmax(counts))
ValLamb = unique[q]
print(f'FreqMax:     {freqMX}')
print(f'Posición: {q}')
print(f'ValMX: {ValLamb}')
...
#Comprobación
print(tabPrueb[posLam,:])

#####################################
########## Generar graficos de Lambda con mayor frecuencia
#####################################
plt.close()
plt.cla()
plt.clf()
...
ruta = './outTXT/ExcelResumBest'+ 'mxLamb'+ Nprueb + denb
fname = ruta + '.csv'
#np.savetxt(fname, resData[:,:4], delimiter =",",fmt='%0.3f, %0.3f, %i, %0.3f')
TAB= pd.DataFrame(resData[:,:4])
TAB[0] = TAB[0].astype('str')
